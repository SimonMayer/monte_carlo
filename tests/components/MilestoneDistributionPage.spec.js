...
describe('MilestoneDistributionPage.vue', () => {
    let wrapper;

    beforeEach(() => {
        wrapper = shallowMount(MilestoneDistributionPage, {
            global: {
                plugins: [store],
            },
        });
    ...
    describe('data properties', () => {
        it('chartId is defined', () => {
            expect(wrapper.vm.chartId).toBe('milestone-distribution-chart');
        });

        it('xTooltip is defined', () => {
            expect(wrapper.vm.xTooltip).toBe('During period {value}');
        });
    ...
    it('passes the correct props to EnsembleChart', async () => {
        const chart = wrapper.findComponent(EnsembleChart);
        expect(chart.props('title')).toBe('Milestone achievement distribution');
